  ...
  list_for_each_entry(file_info, &vtfs_files, list) {
    if (file_info->parent_ino == current_ino) {
      if (count == pos) {
        ...
        if (!dir_emit(ctx, file_info->name, strlen(file_info->name), file_info->ino, type))
          return 0;
        ctx->pos++;
        return 1;
      }
...
int vtfs_create(
  ...
  struct inode* inode = vtfs_get_inode(
      parent_inode->i_sb, NULL, S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO, new_file_info->ino
  );
  if (!inode) {
...
ssize_t vtfs_write(struct file* filp, const char __user* buffer, size_t length, loff_t* offset) {
  ...
  if (required_size > file_info->content.buff_size) {
    ...
    if (new_size > file_info->content.buff_size) {
      memset(new_data + file_info->content.buff_size, 0, new_size - file_info->content.buff_size);
    }
...
int vtfs_link(struct dentry* old_dentry, struct inode* parent_dir, struct dentry* new_dentry) {
  ...
  struct inode* new_inode =
      vtfs_get_inode(parent_dir->i_sb, NULL, old_inode->i_mode, new_file_info->ino);
  if (!new_inode)
    return -ENOMEM;
